...
if (defined $sampleIdList) {
  ...
  Sra::getFastqForSampleIds(\@tmp, "$readsOne", "$readsTwo", $doNotGetFastq, $hasPairedEnds);
} elsif (defined $studyId) {
  Sra::getFastqForStudyId($studyId, $hasPairedEnds, $doNotGetFastq, $deflineVars);
  ...
  Sra::getFastqForStudyRuns(\@runs, $hasPairedEnds, $doNotGetFastq, $deflineVars);
